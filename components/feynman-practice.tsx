...
interface FeynmanPracticeProps {
  textbookId: string
  concepts: string[] // List of key concepts from the textbook
}

export function FeynmanPractice({ textbookId, concepts }: FeynmanPracticeProps) {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      const res = await authFetch('/api/textbook/feynman', {
        method: 'POST',
        dedupeKey: `credits:/api/textbook/feynman:${textbookId}:${selectedConcept}:${explanationSig}`,
      ...
      toast({
        ...
        description: "Could not evaluate your explanation. Please try again.",
        variant: "destructive",
      })
    } finally {
        ...
        <Card className="border-0 shadow-xl overflow-hidden">
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className={`text-5xl font-bold bg-gradient-to-r ${getScoreColor(evaluation.score)} bg-clip-text text-transparent`}>
                {evaluation.score}%
              </div>
            </div>
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-lg overflow-hidden bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-bold text-foreground">The Feynman Technique</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Named after Nobel Prize-winning physicist Richard Feynman, this technique forces you to 
                truly understand a concept by explaining it in simple terms. If you can&apos;t explain it simply, 
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select a concept from your textbook that you want to practice explaining
          </CardDescription>
        </CardHeader>
        <CardContent>
          {concepts.length > 0 ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Explain the concept as if you&apos;re teaching it to a child or someone who knows nothing about the topic.
            Use simple words and avoid jargon.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Textarea
            placeholder="Write your explanation here... Pretend you're explaining to a 10-year-old who is curious and asks 'why?' a lot."
            value={explanation}
