...
async function handler(req: NextRequest, _userId: string) {
  ...
  try {
    ...
    const res = await fetch(`${backend}/cover-letter`, { method: 'POST', body: out })
    ...
    return NextResponse.json({ error: getErrorMessage(e) || 'Failed to proxy cover letter generation' }, { status: 500 })
  }
}

export const POST = requireCredits(3, 'COVER_LETTER', handler)
